...
export async function GET(req: NextRequest) {
  ...
  const header = [
    ...
    "Attendance/100", "Chuyên cần/100", "Phù hợp chuyên ngành/100", "Tổng kết/100",
  ];
  const body = rows.map((r, i) => [
    i + 1, r.student_code || "", r.full_name,
    ...r.cells.map(shortOf),
    r.attendance100, r.diligence ?? "", r.major_fit ?? "", r.tb,
  ]);
  ...
  ws["!cols"] = [{ wch: 5 }, { wch: 12 }, { wch: 22 }, ...sessions.map(() => ({ wch: 4 })), { wch: 14 }, { wch: 13 }, { wch: 20 }, { wch: 12 }];
